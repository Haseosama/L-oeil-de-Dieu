...
const ENVIRONMENTAL_LABELS = Object.freeze({
  ENVIRONMENTAL: Object.freeze({ title: 'ENVIRONNEMENT' }),
  EARTH_WATCH: Object.freeze({ title: 'SURVEILLANCE TERRESTRE' }),
  ACTIVE_EVENTS: Object.freeze({ title: 'ÉVÉNEMENTS ACTIFS' }),
});
...
export const FIRST_RUN_MISSIONS = Object.freeze({
  contacts: Object.freeze({
    ...
    busyText: 'Démarrage des contacts en direct…',
  }),
  'space-missions': Object.freeze({
    kind: 'context',
    contextMode: 'space-missions',
    busyText: 'Ouverture des missions spatiales…',
  }),
  environmental: Object.freeze({
    ...
    busyText: 'Analyse des événements actifs…',
  }),
...
export function initFirstRunExperience({
  ...
  const setBusy = (next, choice = '') => {
    ...
    if (!status) return;
    if (next) status.textContent = FIRST_RUN_MISSIONS[choice]?.busyText || 'En cours…';
  ...
  const onChoice = async (event) => {
    ...
    if (status) {
      ...
      status.textContent = `Impossible d'ouvrir cette mission${detail}. Réessayez ou explorez manuellement.`;
    }
    setBusy(false);
  };
  ...
  const onSuppressChange = (event) => {
    ...
    if (!status) return;
    status.dataset.sticky = 'true';
    status.textContent = 'Ce navigateur bloque le stockage, cela n\'a donc pas pu être enregistré.';
  };

...
import { flyToAustin, flyToCoordinates } from './camera.js';
import { getStartupPosition } from './startupGeolocation.js';
...
async function init() {
  ...
  try {
    ...
    // If no share link state, try the visitor's own location first, falling
    // back to the Austin default when permission is denied, unavailable, or
    // slow to resolve (getStartupPosition never rejects — see its own docs).
    if (!styleManager.hasShareState) {
      loaderStatus.textContent = 'Localisation de votre position...';
      const position = await getStartupPosition();
      if (position) {
        loaderStatus.textContent = 'Envol vers votre position...';
        flyToCoordinates(viewer, position.lon, position.lat);
      } else {
